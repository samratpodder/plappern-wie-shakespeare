...
class Head(nn.Module):
    def __init__(self, head_size, *args, **kwargs) -> None:
    ...
    def forward(self, x):
        ...
        wei = wei * C**-0.5 # scaling to not lose data when softmax
...
class DecoderBlock(nn.Module):
    def __init__(self, n_head, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self.sa_mhead = MultiHeadAttention(num_heads=n_head,head_size=n_embd//n_head)
        self.ffwd = FeedForward()
        self.ln1 = nn.LayerNorm(n_embd)
        self.ln2 = nn.LayerNorm(n_embd)
        
    def forward(self,x):
        x = x + self.sa_mhead(self.ln1(x))
...
class BigramLanguageModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.dcdblks = nn.Sequential(
            DecoderBlock(n_head=4),
            DecoderBlock(n_head=4),
            DecoderBlock(n_head=4),
            DecoderBlock(n_head=4),
            DecoderBlock(n_head=4),
